...
def test_strptime() raises:
    ...
    m = Morrow.strptime("2024-02-29 03:04:05 +0530", "%Y-%m-%d %H:%M:%S %z")
...
def test_humanize_and_dehumanize() raises:
    ...
    assert_equal(
        String(present.dehumanize("in 1 hour and 2 minutes")),
        "2024-01-01T12:02:00.000000+00:00",
    )
    assert_equal(
        String(present.dehumanize("in 1 year and 2 months")),
...
def test_creation_helpers() raises:
    ...
    assert_string_get_raises("2024-02-29T03.5")
    assert_string_get_raises("2024-02-29T0304.5")
    assert_string_get_raises("2024-02-29T03:04.5")
...
def test_flexible_get_creation_helpers() raises:
    ...
    var mixed_upper_meridian = Morrow.get("2023-01-02 03 Pm", "YYYY-MM-DD HH A")
    assert_equal(
        String(mixed_upper_meridian), "2023-01-02T03:00:00.000000+00:00"
    )

    var mixed_lower_meridian = Morrow.get("2023-01-02 03 pM", "YYYY-MM-DD HH a")
    assert_equal(
        String(mixed_lower_meridian), "2023-01-02T03:00:00.000000+00:00"
    )

    var mixed_upper_24_hour_meridian = Morrow.get(
        "2023-01-02 13 Pm", "YYYY-MM-DD HH A"
    )
    assert_equal(
        String(mixed_upper_24_hour_meridian),
        "2023-01-02T13:00:00.000000+00:00",
    )
    ...
    var ascii_control_whitespace_regex = Morrow.get(
        "Mon\fSep\v08 16:41:45\r2014",
        "ddd[\\s+]MMM[\\s+]DD[\\s+]HH:mm:ss[\\s+]YYYY",
    )
    assert_equal(
        String(ascii_control_whitespace_regex),
        "2014-09-08T16:41:45.000000+00:00",
    )

    var optional_whitespace_regex = Morrow.get(
        "MonSep0816:41:452014",
        "ddd[\\s*]MMM[\\s*]DD[\\s*]HH:mm:ss[\\s*]YYYY",
    )
    assert_equal(
        String(optional_whitespace_regex), "2014-09-08T16:41:45.000000+00:00"
    )
    var optional_present_whitespace_regex = Morrow.get(
        "Mon \t Sep 08   16:41:45     2014",
        "ddd[\\s*]MMM[\\s*]DD[\\s*]HH:mm:ss[\\s*]YYYY",
    )
    assert_equal(
        String(optional_present_whitespace_regex),
        "2014-09-08T16:41:45.000000+00:00",
    )
    ...
    var normalized_ascii_control_whitespace = Morrow.get(
        "2013-05-05\fT\v12:30:45",
        "YYYY-MM-DD T HH:mm:ss",
        normalize_whitespace=True,
    )
    assert_equal(
        String(normalized_ascii_control_whitespace),
        "2013-05-05T12:30:45.000000+00:00",
    )
    ...
    assert_equal(
        String(Morrow.get("00", "X")), "1970-01-01T00:00:00.000000+00:00"
    )
    assert_equal(
        String(Morrow.get("0.0", "X")), "1970-01-01T00:00:00.000000+00:00"
    )
    assert_equal(
        String(Morrow.get("-01", "X")), "1969-12-31T23:59:59.000000+00:00"
    )
    assert_equal(
        String(Morrow.get("01.5", "X")), "1970-01-01T00:00:01.500000+00:00"
    )
    assert_get_raises("1", "X")
    assert_get_raises("+1709175845", "X")
    assert_get_raises("1709175845 ", "X")
    assert_get_raises("1e3", "X")
    assert_get_raises("10.", "X")
    assert_equal(
    ...
    assert_get_raises("+1709175845123", "x")
    assert_get_raises("1709175845123 ", "x")
    assert_get_raises("1e3", "x")
